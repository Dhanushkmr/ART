...
1. **Calculate statistics per component (group-wise):**
   ```
...
2. **Normalize each component and apply weights:**
   ```
   For each trajectory:
       For each component k:
           normalized_k = (trajectory.rewards[k] - mean_k) / (std_k + ε)
           weighted_k = normalized_k * reward_weights[k]  # default weight = 1.0
   ```

3. **Sum weighted normalized components:**
   ```
   pre_bn_advantage = sum(weighted_k for all components k)
   ```

4. **Apply batch normalization (during training):**
   ```
   batch_mean = mean(pre_bn_advantage for all samples in batch)
   batch_std = std(pre_bn_advantage for all samples in batch)
   advantage = (pre_bn_advantage - batch_mean) / (batch_std + ε)
   ```

### Reward Scaling

The `scale_rewards` parameter applies to each component individually in GDPO:
...
You can weight different reward components to emphasize certain objectives over others:
...
model.train(
    ...
    _config={
        "scale_rewards": True,
        "reward_weights": {
            "correctness": 2.0,  # Weight correctness more heavily
            "format": 0.5,
            "length": 0.5,
        },
    }
...
If `reward_weights` is not specified, all components are weighted equally (1.0).

### Batch Normalization

GDPO applies batch normalization to the combined advantages during training. This ensures:
- Zero-mean advantages at the batch level
- Consistent gradient scales across batches

This is enabled by default and can be disabled for experimentation:

```python
model.train(
    trajectory_groups,
    _config={
        "gdpo_batch_norm": False,  # Disable batch normalization (default: True)
    }
)
```
